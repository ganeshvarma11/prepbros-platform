...
const SUPPORT_EMAIL = process.env.SUPPORT_TO_EMAIL || "support@prepbros.in";
...
async function handleSupportRequest(
  ...
  if ("error" in validation) {
    return {
      ...
      body: {
        ok: false,
        message: validation.error || "We couldn't submit this request.",
      },
    };
  }
  ...
  if (!persisted && !notified) {
    return {
      ...
      body: {
        ok: false,
        message: `We couldn't deliver your request right now. Please email ${SUPPORT_EMAIL} directly.`,
      },
    };
  }
